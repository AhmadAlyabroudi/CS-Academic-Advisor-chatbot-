  ...
  if (activeMeeting && currentPage !== 'room.html') {
    ...
    widget.innerHTML = `
      <style>
        ...
        .mini-title { font-size: 0.9rem; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px; }
        .mini-tag { font-size: 0.7rem; background: #3b82f6; padding: 2px 6px; border-radius: 4px; }
        .mini-video-preview { width: 100%; height: 140px; background: #0f172a; border-radius: 0.5rem; margin-bottom: 0.8rem; overflow: hidden; position: relative; }
        .mini-video-preview video { width: 100%; height: 100%; object-fit: cover; }
        .mini-controls { display: flex; gap: 0.5rem; margin-top: 0.8rem; }
        .mini-btn { flex: 1; padding: 0.5rem; border-radius: 0.5rem; border: none; cursor: pointer; font-size: 0.8rem; font-weight: 600; transition: 0.2s; }
        .btn-return { background: #3b82f6; color: white; }
        .btn-return:hover { background: #2563eb; }
        .btn-quit { background: rgba(239, 68, 68, 0.1); color: #ef4444; border: 1px solid #ef4444; }
        .btn-quit:hover { background: #ef4444; color: white; }
      </style>
      <div class="mini-header">
        <span class="mini-tag">LIVE PREVIEW</span>
        <div style="display:flex; align-items:center; gap:5px; color:#10b981; font-size:0.8rem;">
          <span class="live-dot-sm" style="background:#10b981; width:8px; height:8px; border-radius:50%; display:inline-block;"></span>
        </div>
      </div>
